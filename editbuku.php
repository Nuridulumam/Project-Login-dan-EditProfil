...
if (isset($_GET['data'])) {
  ...
  //get data buku 
  $sql_m = "SELECT `id_kategori_buku`,`judul`,`pengarang`,`id_penerbit`, `tahun_terbit`,`sinopsis` FROM `buku` WHERE `id_buku`='$id_buku'";
        ...
        <div class="card card-info">
          ...
          <div class="col-sm-10">
            <?php if ((!empty($_GET['notif'])) && (!empty($_GET['jenis']))) { ?>
              <?php if ($_GET['notif'] == "editkosong") { ?> <div class="alert alert-danger" role="alert">Maaf data
          ...
          <form class="form-horizontal" action="konfirmasieditbuku.php" method="post" enctype="multipart/form-data">
            <div class="card-body">
              ...
              <div class="form-group row">
                ...
                <div class="col-sm-7">
                  <select class="form-control" id="penerbit" name="penerbit">
                    ...
                    <?php $sql_t = "SELECT `id_penerbit`,`penerbit` FROM `penerbit` ORDER BY `penerbit`";
                    $query_t = mysqli_query($koneksi, $sql_t);
                    while ($data_t = mysqli_fetch_row($query_t)) {
                      $id_terbit = $data_t[0];
                      $terbit = $data_t[1]; ?> <option value="<?php echo $id_terbit; ?>" <?php if ($id_terbit == $id_penerbit) { ?> selected <?php } ?>><?php echo $terbit; ?></option>
                    <?php } ?>
                  </select>
                </div>
              </div>
              ...
              <div class="form-group row">
                ...
                <div class="col-sm-7">
                  <?php $sql_g = "SELECT `id_tag`,`tag` FROM `tag` ORDER BY `tag`";
                  $query_g = mysqli_query($koneksi, $sql_g);
                  while ($data_g = mysqli_fetch_row($query_g)) {
                    $id_tg = $data_g[0];
                    $tg = $data_g[1]; ?> <input type="checkbox" name="tag[]" value="<?php echo $id_tg; ?>" <?php if (in_array($id_tg, $array_tag)) { ?>checked="checked" <?php } ?>> <?php echo $tg; ?> </br>
                  <?php } ?>
                </div>
              </div>
            </div>
